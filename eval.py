...
def test(data,
    ...
    for batch_i, (batch) in enumerate(tqdm(dataloader, desc=s)):
        img = batch['inp'].to(device,non_blocking=True)
        ...
        with torch.no_grad():
            # Run model
            t = time_synchronized()
            # inf_out, train_out = model(img,batch)  # inference and training outputs
            all = model(img)  # inference and training outputs
            inf_out,train_out = all[0]
