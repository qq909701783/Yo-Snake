...
from utils.snake import snake_cityscapes_utils, snake_config, visualize_utils
import cv2
import numpy as np
import math
...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        labels = []
        for poly in ret['i_gt_py']:
            poly = poly * 4
            poly = np.array(poly,dtype=np.int)
            box = cv2.boundingRect(poly)
            xmin,ymin,xmax,ymax = box[0],box[1],box[0]+box[2],box[1]+box[3]
            labels.append([0,xmin,ymin,xmax,ymax])

        labels = np.array(labels,dtype=np.float32)
        nL = len(labels)
        if nL:
            labels[:, 1:5] = xyxy2xywh(labels[:, 1:5])
            labels[:, [2, 4]] /= inp.shape[2]
            labels[:, [1, 3]] /= inp.shape[1]
        labels_out = torch.zeros((nL, 6))
        if nL:
            labels_out[:, 1:] = torch.from_numpy(labels)

        ret.update({'labelout': labels_out})
        ret.update({'path': path})
        ret.update({'shapes': ((512,512),(1,1),(0,0))})
        return ret
